...
Ally::Ally(Texture* texture, Vector2u imageCount, float switchTime, float speed, Vector2f setPosition, int mode)
...
void Ally::AllyInput(const float& deltaTime, Vector2f gotoPosition)
{
		velocity.x = 0;
		velocity.y = 0;
...
void Ally::Update(const float& deltaTime)
{
	switchTime = 0.1;
...
void Ally::Draw(RenderTarget& target)
{
	target.draw(body);
	//target.draw(hitbox);
	if (isAlive == true) {
